"""
experiments/run_gru.py
----------------------
Trains the GRU classifier with the best hyperparameters found during
grid search (avg_val_f1 = 0.8774 ± 0.0047).
...
2. Builds the GRU model with the best hyperparameters.
...
5. Saves:
     results/gru/best_model.pth
     results/gru/experiment.log

GRU-specific notes
------------------
* GRUs use ~33 % fewer parameters than LSTMs per layer (two gates vs three),
  so they typically converge faster and may generalise better with less data.
* Because there is no cell state, the GRU hidden state directly acts as the
  output; there is no forget-gate bias trick to apply.
* GRUs are often the first choice when training budget is limited: they match
  or beat LSTMs at a fraction of the cost.
"""
...
# ---------------------------------------------------------------------------
# Best hyperparameters (grid search: avg_val_f1 = 0.8774 ± 0.0047)
# ---------------------------------------------------------------------------
HIDDEN_DIM: int   = 256
DROPOUT:    float = 0.5
LR:         float = 1e-3
